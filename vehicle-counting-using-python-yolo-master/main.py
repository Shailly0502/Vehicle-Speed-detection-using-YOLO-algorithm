...
while True:
    ...
    for output in layerOutputs:
        ...
        for detection in output:
            ...
            if confidence > args["confidence"]:
                ...
                # height
              if(classID==3 or classID==2 or classID==5):
    ...
    if len(boxes) > 0:
        ...
        for box in boxes:
            ...
            cv2.waitKey(0)
            '''
            if(y>600 and id not in savedid):
                file = 'SavedVehicles//' + str(frameIndex) + '.jpg'
                cv2.imwrite(file,crop)
                savedid.append(id)
            cv2.line(frame,(250,600),(300,600),(0,255,0),2)
            ...
            if indexIDs[i] in previous:
                ...
                #Speed Calculation
                y_pix_dist = (int(y + (h-y)/2) - int(y2 + (h2-y2)/2))*44
                text_y = "{} y".format(y_pix_dist)
                x_pix_dist = (int(x + (w-x)/2) - int(x2 + (w2-x2)/2))*44
                text_x = "{} x".format(x_pix_dist)
                d_pixels =(math.sqrt(math.pow(x2 - x, 2) + math.pow(y2 - y, 2)))*8.8
                ...
                real_dist=final_pix_dist/ 8.8
                #speed = np.round(1.5 * final_pix_dist,2)
                fps=1
